...
class Table extends Component {
  ...
  async handleFilter(type) {
    ...
    if (type.absent && type.normal && type.overwork && type.underwork) {
      ...
      await this.setState({
        user: _.filter(data, i => i.status !== 'Absent' && i.status !== 'Overwork' && i.status !== 'Underwork')
      })
    } else if (type.absent && !type.normal && type.overwork && type.underwork) {
      await this.setState({
      ...
      await this.setState({
        user: _.filter(data, i => i.status !== 'Absent' && i.status !== 'Normal' && i.status !== 'Underwork')
      })
    } else if (type.absent && type.normal && !type.overwork && type.underwork) {
      await this.setState({
      ...
      await this.setState({
        user: _.filter(data, i => i.status !== 'Overwork' && i.status !== 'Underwork')
      })
    } else if (!type.absent && !type.normal && type.overwork && type.underwork) {
      await this.setState({
      ...
      await this.setState({
        user: _.filter(data, i => i.status !== 'Normal' && i.status !== 'Underwork')
      })
    } else if (!type.absent && type.normal && !type.overwork && type.underwork) {
      await this.setState({
        user: _.filter(data, i => i.status !== 'Absent' && i.status !== 'Overwork')
      })
    } else if (type.absent && !type.normal && !type.overwork && type.underwork) {
      await this.setState({
        user: _.filter(data, i => i.status !== 'Normal' && i.status !== 'Overwork')
      })
    } else if (!type.absent && type.normal && type.overwork && !type.underwork) {
      await this.setState({
        user: _.filter(data, i => i.status !== 'Absent' && i.status !== 'Underwork')
      })
    }
