        ...
        <tr>
          {
            ...
            ? <td>
                <div style={{display: 'flex'}}>
                  <input className="form-control" style={{marginRight:'15px'}} type="text" value={!this.state.current_position_edit ? position.name : this.state.current_position_edit} onChange={this.handleEditPositionChange}/>
                  <button className="btn btn-sm btn-success" onClick={() => this.handleEditPosition(position.id)}><i className="fa fa-check-circle-o" aria-hidden="true"></i></button>
                </div>
              </td>
            : <td>{position.name}</td>
          }
          <td className="action-group">
            <button className="btn btn-sm btn-warning" onClick={() => this.handleEditButton(position.id, position.name)}><i className="fa fa-pencil-square-o" aria-hidden="true"></i></button>
             <button className="btn btn-sm btn-danger" onClick={() => this.handleDeletePosition(position.id)}><i className="fa fa-trash-o" aria-hidden="true"></i></button>
          </td>
        </tr>
      );
    });
              ...
              <Nav tabs>
                <NavItem>
                  <Link to="/setting/position">
                    <NavLink>Update Position</NavLink>
                  </Link>
                </NavItem>
                <NavItem>
                  <Link to="/setting/addposition">
                    <NavLink className="active">Add Position</NavLink>
                  </Link>
                </NavItem>
              </Nav>
            </div>
          ...
          <div className="col-md-6">
             <div className="card">
              ...
              <div className="card-block">
                <input className="form-control" type="text" placeholder="Position Name" value={this.state.new_position_name} onChange={this.handleChangePosition}/>
                <br/>
                <div className="text-center">
                  <button className="btn btn-success" onClick={this.handleAddPosition}>Add</button>
                </div>
              </div>
            </div>
          </div>
