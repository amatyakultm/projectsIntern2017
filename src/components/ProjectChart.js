...
class ProjectChart extends Component {
  ...
  render() {
    const chartData = _.map(this.state.projects, (item, index) => {
      ...
      var listLabelOption = []
      var colorList = []
      var positionList = ['Project Management Officer', 'Frontend Developer', 'Backend Developer', 'Quality Assurance Engineer', 'Business Analyst', 'Designer', 'Mobile Developer', 'HR Director','Technology']
      ...
      return (
        <div key={`div-${index}`} className='col-lg-3 col-md-4 col-sm-6 col-6'>
          <div className='card-box'>
            <h4 className='text-mute2 text-center margin-text' key={`sum-${index}`}>{parseInt(item.sum/1000/60/60)} Hrs</h4>
            ...
            <div className="btn-box-detail">
              <button key={`btn-details-${index}`} onClick={() => this.handleClickDetail(Object.keys(item)[0])} className='btn btn-secondary btn-sm btn-details'>Details</button>
            </div>
          </div>
        </div>
      )
        ...
        <div className='row'>
          <div className='col-sm-12'>
            <div className='btn-group pull-right'>
              <button className='btn btn-danger'><i className='zmdi zmdi-settings'></i>Month</button>
            </div>
            <h4 style={{marginTop:'5px'}}><i className="zmdi zmdi-globe" ></i> Projects</h4>
          </div>
        </div>
